...
md"Normalizing Flows allow transformation of samples from a simple distribution to samples from a complex distribution by applying a series of invertible functions."

# ╔═╡ 0c6e5a98-d161-4592-a7cb-1df16491498d
md" It allows density estimation and statistical inference using machine-learning. That is, to estimate density from data. We are given samples and we want to retrieve
the density function from which the samples were generated."
...
$zₖ = x$ where x is expected to be the distribution the input is a sample of
...
$s_{i} = RNN(z_{i-1}, s_{i-1})$ for $i > 1$"
...
md"To construct such a network, one takes an arbitrary neural network and removes connections until there is no path from input $z_{i}$ to output $(h_{1}, . . . , h_{D})$, i.e., remove connections is by multiplying each weight matrix elementwise with a binary matrix of the same size."
...
md"It is the layer that applies the transformation, whose parameters come from the conditioner, to the input"

# ╔═╡ 4838b626-14ae-4b2d-b6b2-88a1daa62736
md"Transformer - $τ(z_{i}, h_{i}) = α_{i}*z_{i} + β_{i}$ where $α_{i}$ must be non-zero, $h_{i} = {α_{i}, β_{i}}, h_{i} = c(s_{i})$
"
...
md"The function `f` applies the transformer or inverse to the values of z -"
...
md"`train!(rng, ps, data, pᵤ, opt, model)` is used to train the parameters of the conditioner so that the density function may be learnt."

# ╔═╡ 512a4768-9481-422b-aae2-978f2c57bcdf
md"For each data point in `data`, the KL divergence between the base distribution, `pᵤ` and induced distribution, $pᵤ^{*}$ is minimized."
...
md"`expected_pdf(data, pᵤ, A)` takes the base distribution ,affine layer and the value whose density is to be esitmated and outputs the probability density of `data` according to the density function generated."
